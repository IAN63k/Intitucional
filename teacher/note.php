...
<head>
    ...
    if (!isset($_SESSION['rol'])) {
        ...
        if ($_SESSION['rol'] != 2) {
            header('location: ../index.php');
        }else if (isset($_POST["btn-expor"])) { header('location: ./note+.php'); }
        elseif (isset($_POST["btn-pro"])) {header('location: ./note-.php');}
    }
    ?>
</head>
    ...
    <main>
        ...
        <div class="container">
            ...
            <div class="house courses">
                ...
                <button type='submit'  name='btn-expor' >Exportar</button>
                <button type='submit'  name='btn-pro' >Promedio</button>
                </form>"; 
                $qr = mysqli_query($mysqli, "SELECT * from asignatura WHERE id_asignatura='" . $_SESSION["asig"] . "' "); //Realiza el query para obtener el nombre.
            ...
            <div class="second-content" id='reload'><br>
                <?php

                echo "<input type='hidden' name='id_asig' value='" . $_SESSION["asig"] . "'>";
                ...
                while ($datos = mysqli_fetch_assoc($result2)) {
                    echo "<form action='#' method='POST' > 
                             <tr>
                                        <td><span name='nombre'>" . $datos["nombre"] . "</span><hr></td>
                                        <td><input type='text' value='" . $datos["corte_1"] . "' name='corte1' class='modal-input'   ></td>
                                        <td><input type='text' value='" . $datos["corte_2"] . "' name='corte2' class='modal-input'   ></td>
                                        <td><input type='text' value='" . $datos["corte_3"] . "' name='corte3' class='modal-input'   ></td>
                                        <td><input type='text' value='" . $datos["proyecto"] . "' name='proyecto' class='modal-input'></td>
                                        <td><input type='text' value='" . $datos["adicionales"] . "' name='adicionales' class='modal-input' ></td>
                                        <td><input type='text' value='" . $datos["notaFinal"] . "' name='Final' class='modal-input' disabled></td>
                                    
                                        <input type='hidden' name='tb-id' value='" . $datos['id'] . "' >
                                        

                                        <td><button type='submit'  name='btn-save' id='btn-save' class='btn'><svg id='Capa_1' enable-background='new 0 0 512 512' height='40px' viewBox='0 0 512 512' width='50px' xmlns='http://www.w3.org/2000/svg'><g><path d='m495 144v338c0 16.54-13.46 30-30 30h-418c-16.54 0-30-13.46-30-30v-452c0-16.54 13.46-30 30-30h98c38.7 17.32 77.19 23.71 111 24.65 63.88 1.79 111-15.84 111-15.84l123.65 124.62c2.79 2.81 4.35 6.61 4.35 10.57z' fill='#4d5788'/><path d='m495 144v338c0 16.54-13.46 30-30 30h-209v-487.35c63.88 1.79 111-15.84 111-15.84l123.65 124.62c2.79 2.81 4.35 6.61 4.35 10.57z' fill='#313a6b'/><path d='m367 8.81v151.19c0 8.28-6.72 15-15 15h-192c-8.28 0-15-6.72-15-15v-160h207c4 0 7.83 1.6 10.65 4.43z' fill='#f3d652'/><path d='m367 8.81v151.19c0 8.28-6.72 15-15 15h-96v-175h96c4 0 7.83 1.6 10.65 4.43z' fill='#e9bc3e'/><path d='m303 0v96c0 8.28-6.72 15-15 15s-15-6.72-15-15v-96z' fill='#dc455b'/><path d='m431 256v176c0 8.28-6.72 15-15 15h-320c-8.28 0-15-6.72-15-15v-176c0-8.28 6.72-15 15-15h320c8.28 0 15 6.72 15 15z' fill='#dfe5f2'/><path d='m431 256v176c0 8.28-6.72 15-15 15h-160v-206h160c8.28 0 15 6.72 15 15z' fill='#c7cdde'/><path d='m208 335h-48c-8.284 0-15-6.716-15-15s6.716-15 15-15h48c8.284 0 15 6.716 15 15s-6.716 15-15 15z' fill='#313a6b'/></g></svg></button>
                                        </div></td> 
                                    </form>
                                    </tr> 
                                   ";
                ...
                if (isset($_POST["btn-save"])) {
                    ...
                    if (is_numeric($_POST["corte1"]) && is_numeric($_POST["corte2"]) && is_numeric($_POST["corte3"]) && is_numeric($_POST["proyecto"]) && is_numeric($_POST["adicionales"])) { // Trabajamos con valores numericos.
                        ...
                        $c3 = $_POST["corte3"];
                        $c4 = $_POST["proyecto"];
                        $c5 = $_POST["adicionales"];
                        $final = 0;

                        if (($c1 >= 0 && $c1 <= 5) && ($c2 >= 0 && $c2 <= 5) && ($c3 >= 0 && $c3 <= 5) && ($c4 >= 0 && $c4 <= 5) && ($c5 >= 0 && $c5 <= 5)) {

                            $final = $c1 * 0.2 + $c2 * 0.2 + $c3 * 0.25 + $c4 * 0.2 + $c5 * 0.15;
                            $querry =  mysqli_query($mysqli, "UPDATE notas SET  corte_1='$c1', corte_2='$c2', corte_3='$c3', proyecto='$c4', adicionales='$c5', notaFinal='$final' WHERE id_estudiante = '" . $id . "' AND  id_asig = '" . $_SESSION["asig"] . "'  ");
                        } else {
                            echo "<script>alert('Uno de los valores ingresados no es valido 😯')</script>";
                        }
                    }
                }
