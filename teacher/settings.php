    ...
    <main>
        ...
        <div class="container_sett">
                    ...
                    <div class="user_sett">
                        <h1>Usuario:
                            ...
                            while ($datos = mysqli_fetch_assoc($usr)) {
                                echo "Docente";
                                $SESSION["tabla"] = $datos["usuario"];
                            } ?>
                        </h1>
                    </div>
                    <div class="profile_sett">
                        
                        <img src="<?php  $result = mysqli_query($mysqli, "SELECT * from  " . $SESSION["tabla"] . " WHERE id = '" . $_SESSION["id"] . "' "); while ($datos = mysqli_fetch_assoc($result)) {
                            echo "" . $datos["profile"] . "";
                        } ?>" alt="IMG profile">

                        <!-- Imprimir: Nombre del usuario -->
                        <input type="text" name="nombre" id="" value='<?php echo "" . $_SESSION["nombre"] . ""; ?>'>
                    </div>


                    <div class="camp_sett">
                        <!-- Imprimir: Cedula del usuario-->
                        <input type="text" name="id" id="" value='<?php echo "" . $_SESSION["id"] . ""; ?>'>

                        <!-- Imprimir: Email del usuario-->
                        <input type="email" name="email" value="
                        <?php $result = mysqli_query($mysqli, "SELECT * from " . $SESSION["tabla"] . " WHERE id = '" . $_SESSION["id"] . "'");
                        while ($datos = mysqli_fetch_assoc($result)) {
                            echo "" . $datos["email"] . "";
                        } ?>">

                        <!-- Imprimir: Contraseña del usuario-->
                        <input type="password" name="contraseña" value="<?php $result = mysqli_query($mysqli, "SELECT * from " . $SESSION["tabla"] . " WHERE id = '" . $_SESSION["id"] . "' ");
            ...
            if (isset($_POST["submit"])) {
                ...
                $querry =  mysqli_query($mysqli, "UPDATE " . $SESSION["tabla"] . "  SET nombre='" . $nombre . "', id='" . $id . "', email='" . $email . "', contraseña='" . $password . "', profile='" . $profile . "' WHERE id='" . $_SESSION["id"] . "' ");
