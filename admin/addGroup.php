...
<head>
    ...
    <title>Opciones de grupos</title>
    <?php
        ...
        <div class="container">
            ...
            <div class="principal-content">
                ...
                <form action="#" method="POST">
                    ...
                    <div class="div-add-2"><button class="btn-add-2" name="btn-add"><img src="../img/plus.svg" alt="Añadir asignatura">
                        ...
                        <figcaption id="figcapt">Añadir grupo</figcaption>
                    </div>
                </form>
            </div>
            <br>
            ...
            <div class='second-content'>
                ...
                while ($datos = mysqli_fetch_assoc($result)) {
                    ...
                    echo "</td>
                                <form action='#' method='POST'>
                                    <input type='hidden' name='tb-id' value='" . $datos['id_gp'] . "'>
                                    <input type='hidden' name='tb-nombre' value='" . $datos['grupo'] . "'>
            ...
            ";
                ...
                if (isset($_POST["btn-delete"])) {
                    $gp = $_POST["tb-nombre"];
                    mysqli_query($mysqli, "DELETE FROM grupos WHERE grupo='$gp'");
                    echo "<script>alert('Se ha eliminado el grupo $gp satisfatoriamente, refresque la pagina para ver los cambios.')</script>";
                }
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><input type='text' name='modal-nombre' class='modal-input' value='" . $_POST["tb-nombre"] . "'></td>
                                                    <input type='hidden' name='modal-id-hidden' value=" . $_POST['tb-nombre'] . ">
                                    </tbody>
                                </table>
                ...
                if (isset($_POST["update-btn"])) {
                    ...
                    if (!empty($id) || !empty($nombre)) {
                        ...
                        $querry =  mysqli_query($mysqli, "UPDATE grupos SET  id_gp='$id', grupo='$nombre' WHERE grupo=" . $_POST['modal-id-hidden'] . " ");
                    ...
                    while ($datos = mysqli_fetch_assoc($result)) {
                        ...
                        echo "
                            <tr>
                                    ...
                                    <td>" . $datos['grupo'] . "</td>
                                    <td>";
                        $result2 = mysqli_query($mysqli, "SELECT * from asignatura WHERE  id_grupo ='" . $datos["id_gp"] . "'");
                        while ($datos2 = mysqli_fetch_assoc($result2)) {
                            echo "" . $datos2["asig_nombre"] . "<br>";
                        }
                        echo "</td>
                    ...
                    <div id='#open-modal' class='modal-window'>
                        ...
                        <div class='modal-close'></div>
                            <table class='table'>
                                ...
                                <tbody>
                                    <tr>
                                        <td><input type='number' name='modal-id' class='modal-input' placeholder='Indique la id del grupo'>
                                        <select class='modal-input'>
                                        <option label='Desplegue para ver las ID ya creadas'></option>";
                $result = mysqli_query($mysqli, 'SELECT * from grupos');
                while ($datos = mysqli_fetch_array($result)) {
                    echo "<option class='modal-input' disabled>" . $datos["id_gp"] . " - " . $datos["grupo"] . "</option>";
                }
                echo "</select>
                                        </td>
                                        <td><input type='text' name='modal-nombre' class='modal-input' placeholder='Nombre del grupo'></td>
                                    </tr>    
                                </tbody>
                            </table>
                            <div class='update-btn'>
            ...
            if (isset($_POST["update-btn-add"])) {
                ...
                $id_gp = $_POST["modal-id"];
                $grupo = $_POST["modal-nombre"];
                #almacena

                if (!empty($id_gp) && !empty($grupo)) {
                    #Si no estan vacios ejecuta el Insert.

                    $querry =  mysqli_query($mysqli, "INSERT INTO grupos (id_gp, grupo) VALUES ('$id_gp', '$grupo'); ");
                    echo "<script>alert('Grupo: $grupo creado. Porfavor refresque la pagina.')</script>";
                } else {
                    echo "<script>alert('Existe un campo vacio, por ende no se realizaran cambios')</script>";
                }
            }
