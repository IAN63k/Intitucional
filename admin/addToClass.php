        ...
        <div class="container">
            ...
            <div class="principal-content">
                ...
                <form action="#" method="POST">
                    <input type="text" class="input" placeholder="Indique el nombre del estudiante : " name="search">
            ...
            <div class='second-content'>
                ...
                $result = mysqli_query($mysqli, "SELECT * from estudiantes  ORDER BY nombre ASC");
                while ($datos = mysqli_fetch_assoc($result)) {
                    echo "
                            <tr>
                                    <td>" . $datos['nombre'] . "</td>
                                    <td>" . $datos['id'] . "</td>
                                    <td>";
                    $result2 = mysqli_query($mysqli, "SELECT * from notas,asignatura WHERE  notas.id_asig=asignatura.id_asignatura AND id_estudiante ='" . $datos["id"] . "'  ORDER BY asignatura.asig_nombre ASC ");
            ...
            ";
                ...
                if (isset($_POST["btn-delete"])) {
                    $id = $_POST["tb-id"];
                    echo "<script>alert('Estamos tabajando en esto!🙇‍♂️.')</script>";
                }
                ...
                if (isset($_POST['btn-edit'])) {
                    ...
                    <form action='#' method='POST'>
                        ...
                        <div id='#open-modal' class='modal-window'>
                            ...
                            <h3>Ingrese la cantidad de materias que vera este estudiante.</h3>
                                <table class='table '>
                                    <tbody>
                                        <tr>
                                            <td><input type='number' name='modal-cantidad' class='modal-input' ></td>
                ...
                if (isset($_POST["update-btn"])) {
                    ...
                    # Muestra el modal con los inputs.
                    echo "
                    <form action='#' method='POST'>
                        <link rel='stylesheet' href='./style.css'>
                        <div id='#open-modal' class='modal-window'>
                            <div>
                            <div class='modal-close'></div>
                            <h3>Ingrese las materias.</h3>
                                <table class='table '>
                                    <tbody>
                                       ";
                    for ($i = 1; $i <= $_POST["modal-cantidad"]; $i++) {
                        # Imprime la cantidad de campos indicados. Para las asignaturas.
                        echo "  <tr> 
                                                    <select class='modal-input' name='select-group$i'>";
                        $result = mysqli_query($mysqli, "SELECT * from asignatura,grupos WHERE asignatura.id_grupo = grupos.id_gp");
                        while ($datos = mysqli_fetch_array($result)) {
                            echo "<option class='modal-input' value=' " . $datos["id_asignatura"] . " '>" . $datos["asig_nombre"] . " - " . $datos["grupo"] . "</option>";
                        }
                        echo "
                                                    </select>
                                                        </tr>";
                    ...
                    echo "
                                ...
                                <div class='update-btn'>
                                    <input type='hidden' name='cant' value='" . $_POST["modal-cantidad"] . "'>
                                    <input type='hidden' name='modal-hidden' value=" . $_POST['modal-id-hidden'] . ">
                                    <button type='submit' name='btn-asig' class='updatebtn'>Agregar</button>
                                </div>
                            </div>
                        </div>
                    </form>
                ...
                if (isset($_POST["btn-asig"])) {
                    #Si acciona desde el modal para añadir asignaturas

                    $cant = $_POST["cant"]; // almacenamos la cantidad de asignaturas.

                    for ($i = 1; $i <= $cant; $i++) {
                        # recorre los name...   
                        if (!empty($_POST["select-group$i"])) {
                            # Aqui creamos las relaciones entre estudiante y asignatura...
                            $querry =  mysqli_query($mysqli, "INSERT INTO notas (id_asig, id_estudiante, corte_1, corte_2, corte_3, proyecto, adicionales, notaFinal, llave ) VALUES ('" . $_POST["select-group$i"] . "', '" . $_POST["modal-hidden"] . "', '', '', '', '', '', '', ''); ");
                        } else {
                            # si esta vacio no se ejecuta...
                            break;
                        }
                    }
                    echo "<script>alert('Materias registradas, porfavor refresque la pagina.')</script>";
                }
                        ...
                        <div id='#open-modal' class='modal-window'>
                            ...
                            <div class='modal-close'></div>
                                <table class='table'>
                                    ...
                                    <tbody>
                                        <tr>
                                            <td><input type='text' name='modal-nombre' class='modal-input' placeholder='Nombre estudiante'></td>
                                            <td><input type='number' name='modal-id' class='modal-input' placeholder='Cedula'></td>
                                            <td><input type='email' name='modal-email' class='modal-input' placeholder='Correo'></td>
                                            <td><input type='text' name='modal-contraseña' class='modal-input' placeholder='Contraseña provisional'></td>
                                    </tbody>
                                </table>
                                <div class='update-btn'>
                ...
                if (isset($_POST["update-btn-add"])) {
                    ...
                    if (!empty($nombre) || !empty($id) || !empty($email) || !empty($password)) {
                        ...
                        $querry =  mysqli_query($mysqli, "INSERT INTO estudiantes (nombre, id, email, contraseña, course, id_rol ) VALUES ('$nombre', '$id', '$email', '$password', '', '3'); ");
                        echo "<script>alert('Estudiante registrado, porfavor refresque la pagina.')</script>";
                    } else {
            ...
            <tbody>";
                    $result = mysqli_query($mysqli, "SELECT * from estudiantes WHERE id = $search");
                    while ($datos = mysqli_fetch_array($result)) {
                        echo "
                    <tr>
                        <td>" . $datos['nombre'] . "</td>
                        ...
                        <td>" . $datos['id'] . "</td>
                        <td>";
                        $result2 = mysqli_query($mysqli, "SELECT * from notas,asignatura WHERE  notas.id_asig=asignatura.id_asignatura AND id_estudiante ='" . $datos["id"] . "' ");
                        while ($datos2 = mysqli_fetch_assoc($result2)) {
                            echo "" . $datos2["asig_nombre"] . "<br>";
                        }
                        echo "</td>
