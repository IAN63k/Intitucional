    ...
    <main>
        ...
        <div class="container">
            ...
            <div class="principal-content">
                ...
                <p>Asignaturas
                    <button type="submit" name="optionshbtn" class="optionshbtn"><a href="./courseDetails.php">Cambiar a vista</a></button>
                    <br>
                    <br>
                <form action="#" method="POST">
                    <input type="text" class="input" placeholder="Indique la id del docente: " name="search">
                    <button type="submit" name="searchbtn" class="searchbtn">Buscar</button>
                    <div class="div-add-2"><button class="btn-add-2" name="btn-add"><img src="../img/plus.svg" alt="Añadir asignatura">
                        </button>
                        <figcaption id="figcapt">Añadir asignatura</figcaption>
                    </div>
                </form>
            </div>
            <br>
            ...
            <div class='second-content'>
                ...
                $result = mysqli_query($mysqli, "SELECT * from asignatura,docentes,grupos WHERE asignatura.id_docente = docentes.id AND  asignatura.id_grupo = grupos.id_gp ORDER BY docentes.nombre ASC");
                while ($datos = mysqli_fetch_assoc($result)) {
                    ...
                    echo "
                            <tr>
                                ...
                                <form action='#' method='POST'>
                                    ...
                                    <input type='hidden' name='tb-id' value='" . $datos['id_docente'] . "'>
                                    <input type='hidden' name='tb-nombre' value='" . $datos['asig_nombre'] . "'>
                                    <input type='hidden' name='tb-grupo' value='" . $datos['grupo'] . "'>
                                    <input type='hidden' name='tb-id_asig' value='" . $datos['id_asignatura'] . "'>
            ...
            ";
                ...
                if (isset($_POST["btn-delete"])) {
                    $id_asig = $_POST["tb-id_asig"];
                    echo "<script>alert('Asignatura eliminada, refresque la pagina para ver los cambios.')</script>";
                    mysqli_query($mysqli, "DELETE FROM asignatura WHERE id_asignatura='$id_asig'");
                }
                        ...
                        <div id='#open-modal' class='modal-window'>
                            ...
                            <div class='modal-close'></div>
                                <table class='table'>
                                    ...
                                    <tbody>
                                        <tr>
                                            <td><input type='text' name='modal-id' class='modal-input' value='" . $_POST["tb-id"] . "'></td>
                                            <td><input type='text' name='modal-nombre' class='modal-input' value='" . $_POST["tb-nombre"] . "'></td>
                                            <td>
                                            <select class='modal-input' name='modal-grupo'>
                                                <option>Actual: " . $_POST["tb-grupo"] . "</option>";
                    $result = mysqli_query($mysqli, 'SELECT * from grupos');
                    while ($datos = mysqli_fetch_array($result)) {
                        echo "<option value='" . $datos["id_gp"] . "'>" . $datos["id_gp"] . " - " . $datos["grupo"] . "</option>";
                    }
                    echo "</select>
                                                    </td>
                                                    <input type='hidden' name='modal-id-hidden' value=" . $_POST['tb-id_asig'] . ">
                                    </tbody>
                                </table>
                ...
                if (isset($_POST["update-btn"])) {
                    ...
                    $grupo = $_POST["modal-grupo"];
                    # almacena
                    if (!empty($id) && !empty($nombre) && !empty($grupo)) {
                        # Si no estan vacios ejecuta el Update.
                        $querry =  mysqli_query($mysqli, "UPDATE asignatura SET  id_docente='$id', asig_nombre='$nombre', id_grupo='$grupo' WHERE id_asignatura=" . $_POST['modal-id-hidden'] . " ");
                ...
                if (empty($_POST["search"])) {
                    ...
                    echo "<div class='second-content'>
                        <table class='table'>
                            ...
                            <tbody>";
                    $result = mysqli_query($mysqli, "SELECT * from asignatura WHERE id_asignatura = $search");
                    while ($datos = mysqli_fetch_array($result)) {
                        echo "
                                <tr>
                                    <td>" . $datos['id_asignatura'] . "</td>
                                    <td>" . $datos['asig_nombre'] . "</td>
                                </tr>
                            ";
                    }
                        ...
                        <div class='modal-close'></div>
                            <table class='table'>
                                ...
                                <tbody>
                                    <tr>
                                        <td>
                                        <select class='modal-input' name='select-teacher'>";
                $result = mysqli_query($mysqli, 'SELECT * from docentes');
                while ($datos = mysqli_fetch_array($result)) {
                    echo "<option class='modal-input' value=' " . $datos["id"] . " '>" . $datos["nombre"] . " - " . $datos["id"] . "</option>";
                }
                echo "</select>
                                        </td>
                                        <td><input type='text' name='modal-asig' class='modal-input' placeholder='Nombre asignatura'></td>
                                        <td>
                                        <select class='modal-input' name='select-group'>";
                $result = mysqli_query($mysqli, 'SELECT * from grupos');
                while ($datos = mysqli_fetch_array($result)) {
                    echo "<option class='modal-input' value=' " . $datos["id_gp"] . " '>" . $datos["grupo"] . "</option>";
                }
                echo "</select>
                                        </td>
                                </tbody>
            ...
            if (isset($_POST["update-btn-add"])) {
                ...
                $docente = $_POST["select-teacher"];
                $asignatura = $_POST["modal-asig"];
                $grupo = $_POST["select-group"];
                #almacena

                if (!empty($docente) && !empty($asignatura) && !empty($grupo)) {
                    #Si no estan vacios ejecuta el Insert.

                    $querry =  mysqli_query($mysqli, "INSERT INTO asignatura (id_docente, asig_nombre, id_grupo, id_asignatura) VALUES ('$docente', '$asignatura', '$grupo', ''); ");
                    echo "<script>alert('Asignatura $asignatura, creada. Porfavor refresque la pagina.')</script>";
                } else {
                    echo "<script>alert('Existe un campo vacio, por ende no se realizaran cambios')</script>";
                }
            }
